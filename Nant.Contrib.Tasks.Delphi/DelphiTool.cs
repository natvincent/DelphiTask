    ...
    public class DelphiTool : ExternalProgramBase
    {
        ...
        private string findExecutable()
        {
            if (Version != "1")
            {
                ...
                throw new BuildException(string.Format("Unable to locate Delphi tool \"{0}\"", this.ToolName), this.Location);
            }
            else
            {
                return searchPath();
        ...
        protected override void PrepareProcess(Process process)
        {
            ...
            if (_versionString == "1")
            {
                //process.StartInfo.EnvironmentVariables[@"PATH"] = ShortenPathVariable(process.StartInfo.EnvironmentVariables[@"PATH"]);
                //Log(Level.Debug, string.Format(@"New PATH={0}", process.StartInfo.EnvironmentVariables[@"PATH"]));
                process.StartInfo.EnvironmentVariables[@"PATH"] = GetShortFilename(Path.GetDirectoryName(process.StartInfo.FileName));
                //process.StartInfo.FileName = GetShortFilename(process.StartInfo.FileName);
                process.StartInfo.FileName = Path.GetDirectoryName((new System.Uri(Assembly.GetExecutingAssembly().CodeBase).AbsolutePath)).Replace("%20", " ") + Path.DirectorySeparatorChar + "run16bit.exe";
                process.StartInfo.Arguments = ProgramFileName + " " + process.StartInfo.Arguments;
        ...
        {
            Log(Level.Debug, @"ExecuteTask()");
            string oldPath = Environment.GetEnvironmentVariable(@"PATH");
            try
            {
                if (this._version == 1)
                {
                    Log(Level.Debug, @"condensing PATH");
                    string[] paths = oldPath.Split(Path.PathSeparator);
                    StringBuilder builder = new StringBuilder();

                    foreach (string path in paths)
                    {
                        builder.Append(GetFilename(path));
                        builder.Append(Path.PathSeparator);
                    }

                    if (!SetEnvironmentVariable("PATH", builder.ToString())) 
                    {
                        throw new Exception(@"Shit!");
                    }
                    Log(Level.Debug, string.Format(@"New PATH={0}", Environment.GetEnvironmentVariable(@"PATH")));
                }
                base.ExecuteTask();
            }
            finally
            {
                ...
                Log(Level.Debug, @"end ExecuteTask()");
            }
        }*/
        ...
        [StringValidator(
            AllowEmpty = true,
            Expression = "1|2|3|4|5|6|7|2005|2006|2007|2009|2010|XE|XE2|XE3|XE4|XE5|XE6|XE7",
